...
scale = open_scale_serial_matlab(scale_port, cfg);
...
fprintf(fid, ['timestamp,trial,duration_ms,repetition,dry_reward,baseline_g,post_g,' ...
...
if isfield(cfg, 'DRY_REWARD') && cfg.DRY_REWARD
    fprintf('DRY_REWARD is true: goodmonkey will be skipped and the pump will stay off.\n');
end
...
for trial_idx = 1:numel(trial_duration_ms)
    ...
    dry_reward = isfield(cfg, 'DRY_REWARD') && cfg.DRY_REWARD;
    if dry_reward
        fprintf('  DRY_REWARD: skipping goodmonkey(%d), waiting %.3f s\n', ...
            duration_ms, duration_ms / 1000);
        ml_pause_seconds(duration_ms / 1000);
    else
        try
    ...
    fprintf(fid, '%s,%d,%d,%d,%d,%.6f,%.6f,%.6f,%.3f,%s,%s,%s,%s\n', ...
